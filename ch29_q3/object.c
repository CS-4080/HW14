...
ObjMethodChain* newMethodChain() {
  ObjMethodChain* chain = ALLOCATE_OBJ(ObjMethodChain, OBJ_METHOD_CHAIN);
  chain->count = 0;
  chain->capacity = 0;
  chain->methods = NULL;
  return chain;
...
void appendMethod(ObjMethodChain* chain, ObjClosure* method) {
  if (chain->capacity < chain->count + 1) {
    int old = chain->capacity;
    chain->capacity = GROW_CAPACITY(old);
    chain->methods = GROW_ARRAY(ObjClosure*, chain->methods, old, chain->capacity);
  }
  chain->methods[chain->count++] = method;
}

ObjMethodChain* cloneMethodChain(ObjMethodChain* original) {
  ObjMethodChain* chain = newMethodChain();
  for (int i = 0; i < original->count; i++) {
    appendMethod(chain, original->methods[i]);
  }
  return chain;
}
